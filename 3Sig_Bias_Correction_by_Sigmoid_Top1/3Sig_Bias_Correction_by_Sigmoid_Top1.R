...
# File: 3Sig_Bias_Correction_by_Sigmoid_Top1
...
# Function: 
# Output: Three model files "clf_sig_high_i0_seedxxxx.rds", "clf_sig_all_i1_seedxxxx.rds" , and "clf_sig_all_i2_seedxxxx.rds"
...
directory <- getwd()
source("3Sig_Functions.R") # custom functions
...
for (i in 1:num_rerun) 
  {
    ...
    ###########
    # --- Caret-compatible sigmoid model (1-row dummy tuning) ---
    sigmoid_caret_model <- list(
      label = "Sigmoid (SSlogis via nls)",
      library = NULL,
      type = c("Regression"),
      # Provide ONE dummy parameter so the grid has 1 row
      parameters = data.frame(parameter = "dummy", class = "numeric", label = "dummy"),
      grid = function(x, y, len = NULL, search = "grid") data.frame(dummy = 1),
      fit = function(x, y, wts, param, lev, last, classProbs, ...) {
        df <- data.frame(x1 = as.numeric(x[, 1]), y = as.numeric(y))
        df <- df[is.finite(df$x1) & is.finite(df$y), , drop = FALSE]
        
        fit <- try(nls(y ~ SSlogis(x1, Asym, xmid, scal), data = df), silent = TRUE)
        if (inherits(fit, "try-error")) {
          Asym0 <- max(df$y, na.rm = TRUE)
          xmid0 <- stats::median(df$x1, na.rm = TRUE)
          scal0 <- stats::sd(df$x1, na.rm = TRUE); if (!is.finite(scal0) || scal0 == 0) scal0 <- 1
          fit <- nls(y ~ Asym/(1 + exp((xmid - x1)/scal)),
                     data = df,
                     start = list(Asym = Asym0, xmid = xmid0, scal = scal0))
        }
        fit
      },
      predict = function(modelFit, newdata, submodels = NULL) {
        x1 <- as.numeric(newdata[, 1])
        predict(modelFit, newdata = data.frame(x1 = x1))
      },
      prob = NULL,
      sort = function(x) x
    )
    
    # --- CV control with explicit seeds (optional but robust) ---
    # For method='cv', caret expects length(seeds) == number_of_folds + 1,
    # and each fold's seed vector length == nrow(tuneGrid) == 1 here.
    set.seed(seed_i)
    seeds_list <- vector("list", inner_cv + 1)
    for (i in seq_len(inner_cv)) seeds_list[[i]] <- sample.int(1e6, 1)
    seeds_list[[inner_cv + 1]] <- sample.int(1e6, 1)
    
    rfControl <- trainControl(
      method = "cv",
      number = inner_cv,
      verboseIter = TRUE,
      summaryFunction = mse_summary,
      seeds = seeds_list
    )
    
    # --- Train using ONLY the desired column, keep object name the same ---
    options(expressions = 100000)
    cat("Starting initial training on high-intensity data with a sigmoid model...\n")
    set.seed(seed_i)
    
    used_cols <- c("intensity_over_TIC_smoothed")
    x_train_high <- Matrix_training_high[, used_cols, drop = FALSE]
    
    clf_sig_high <- caret::train(
      x = x_train_high,
      y = DataInt_training_high,
      method = sigmoid_caret_model,
      metric = "MSE",
      trControl = rfControl,
      tuneGrid = data.frame(dummy = 1)  # must match model$parameters/grid (1 row)
    )
    
    cat("Initial sigmoid training completed.\n")
    
    saveRDS(clf_sig_high, file = paste0("clf_sig_high_i0_seed", seed_i, ".rds"))
    
    # Importance is not defined for a 1-feature sigmoid; keep it non-fatal
    importance_high <- tryCatch(
      { as.data.frame(importance(clf_sig_high$finalModel)) },
      error = function(e) data.frame(Warning = "No feature importance for sigmoid model.")
    )
    print(importance_high)
    
    # Predictions: unchanged except for the column subset
    predictions_high_training <- predict(clf_sig_high, newdata = Matrix_training_high[, used_cols, drop = FALSE])
    predictions_high_test     <- predict(clf_sig_high, newdata = Matrix_test_high[, used_cols, drop = FALSE])
    ...
    # For low intensity data below the cutoff, the initial clf_sig_high modell is used to predict their true intensity
    result_training <- process_data(Matrix_training_low, training_low_info, clf_sig_high)
    ...
    result_test <- process_data(Matrix_test_low, test_low_info, clf_sig_high)
    ...
    # Testing clf_sig_high on low data
    predictions_low_training <- predict(clf_sig_high, newdata = Matrix_training_low[, used_cols, drop = FALSE])
    predictions_low_test <- predict(clf_sig_high, newdata = Matrix_test_low[, used_cols, drop = FALSE])
    ...
    predictions_all_training <- predict(clf_sig_high, newdata = Matrix_training_all)
    predictions_all_test <- predict(clf_sig_high, newdata = Matrix_test_all)
    ...
    # rm(clf_sig_high) # remove clf_sig_high to save RAM, if needed can use readRDS to load back manually
    ...
    # set.seed(seed_i)
    # clf_sig_all <- train(x = Matrix_training_all, 
    ...
    # saveRDS(clf_sig_all, file = paste0("clf_sig_all_i1_seed", seed_i, ".rds"))
    # # Extract and print importance
    # importance_all <- as.data.frame(importance(clf_sig_all$finalModel))
    ...
    # --- Train using ONLY the desired column, keep object name the same ---
    options(expressions = 100000)
    cat("Starting iteration 1 training on all-intensity data with a sigmoid model...\n")
    set.seed(seed_i)
    

    x_train_all <- Matrix_training_all[, used_cols, drop = FALSE]
    
    clf_sig_all <- caret::train(
      x = x_train_all,
      y = DataInt_training_all,
      method = sigmoid_caret_model,
      metric = "MSE",
    ...
    predictions_high_training <- predict(clf_sig_all, newdata = Matrix_training_high)
    predictions_high_test <- predict(clf_sig_all, newdata = Matrix_test_high)
    ...
    # For low intensity data below the cutoff, the initial clf_sig_all modell is used to predict their true intensity
    result_training <- process_data(Matrix_training_low, training_low_info, clf_sig_all)
    Matrix_training_low_m0 <- result_training$Matrix_m0
    Matrix_training_low_m1 <- result_training$Matrix_m1
    Matrix_training_low_m2 <- result_training$Matrix_m2
    training_low_info_m0 <- result_training$info_m0
    ...
    training_low_info_m2 <- result_training$info_m2
    training_low_info_pseudoL <- result_training$info_pseudoL
    result_test <- process_data(Matrix_test_low, test_low_info, clf_sig_all)
    Matrix_test_low_m0 <- result_test$Matrix_m0
    Matrix_test_low_m1 <- result_test$Matrix_m1
    Matrix_test_low_m2 <- result_test$Matrix_m2
    test_low_info_m0 <- result_test$info_m0
    ...
    # Testing clf_sig_all on low data
    predictions_low_training <- predict(clf_sig_all, newdata = Matrix_training_low)
    predictions_low_test <- predict(clf_sig_all, newdata = Matrix_test_low)
    ...
    predictions_all_training <- predict(clf_sig_all, newdata = Matrix_training_all)
    predictions_all_test <- predict(clf_sig_all, newdata = Matrix_test_all)
    ...
    # set.seed(seed_i)
    # clf_sig_all_i2 <- train(x = Matrix_training_all, 
    #                     y = DataInt_training_all, 
    #                     method = "rf", 
    #                     metric = "MSE", 
    #                     trControl = rfControl, 
    ...
    # cat("Initial training completed.\n")
    # # Save the initial model
    # saveRDS(clf_sig_all_i2, file = paste0("clf_sig_all_i2_seed", seed_i, "_2cRF.rds"))
    # # Extract and print importance
    # importance_all <- as.data.frame(importance(clf_sig_all_i2$finalModel))
    # print(importance_all)
    ...
    # --- Train using ONLY the desired column, keep object name the same ---
    options(expressions = 100000)
    cat("Starting iteration 2 training on all-intensity data with a sigmoid model...\n")
    set.seed(seed_i)
    

    x_train_all <- Matrix_training_all[, used_cols, drop = FALSE]
    
    clf_sig_all_i2 <- caret::train(
      x = x_train_all,
      y = DataInt_training_all,
      method = sigmoid_caret_model,
      metric = "MSE",
    ...
    saveRDS(clf_sig_all_i2, file = paste0("clf_sig_all_i2_seed", seed_i, ".rds"))
    
    
    # Test iteration 1 all intensity model accuracy
    predictions_high_training <- predict(clf_sig_all_i2, newdata = Matrix_training_high)
    predictions_high_test <- predict(clf_sig_all_i2, newdata = Matrix_test_high)
    ...
    # For low intensity data below the cutoff, the initial clf_sig_high modell is used to predict their true intensity
    result_training <- process_data(Matrix_training_low, training_low_info, clf_sig_all_i2)
    Matrix_training_low_m0 <- result_training$Matrix_m0
    Matrix_training_low_m1 <- result_training$Matrix_m1
    Matrix_training_low_m2 <- result_training$Matrix_m2
    training_low_info_m0 <- result_training$info_m0
    ...
    training_low_info_m2 <- result_training$info_m2
    training_low_info_pseudoL <- result_training$info_pseudoL
    result_test <- process_data(Matrix_test_low, test_low_info, clf_sig_all_i2)
    Matrix_test_low_m0 <- result_test$Matrix_m0
    Matrix_test_low_m1 <- result_test$Matrix_m1
    Matrix_test_low_m2 <- result_test$Matrix_m2
    test_low_info_m0 <- result_test$info_m0
    ...
    # Testing clf_sig_all_i2 on low data
    predictions_low_training <- predict(clf_sig_all_i2, newdata = Matrix_training_low)
    predictions_low_test <- predict(clf_sig_all_i2, newdata = Matrix_test_low)
    ...
    predictions_all_training <- predict(clf_sig_all_i2, newdata = Matrix_training_all)
    predictions_all_test <- predict(clf_sig_all_i2, newdata = Matrix_test_all)
    ...
    # ###############################################################
    # # Next, we will calculate permutation importance of clf_sig_all_i2
    ...
    # 
    # original_predictions <- predict(clf_sig_all_i2$finalModel, Matrix_test_all) # only use test data to calculate permutation score
    ...
    #   permuted_predictions <- predict(clf_sig_all_i2$finalModel, permuted_data)
    ...
    # )
    # write_xlsx(importance_sheets, path = paste0("clf_sig_all_i2_seed", seed_i, "_2cRF_importance_rank_fig3.xlsx"))
    # 
    # # Plot importance
